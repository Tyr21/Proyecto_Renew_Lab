...
export function serviceColorClasses(serviceId: string): string {
	const palette: Record<string, string> = {
		camara_hiperbarica: "bg-sky-100 border-sky-400 text-sky-950",
		sueroterapia: "bg-sky-100 border-sky-400 text-sky-950",
	};
	return palette[serviceId] ?? "bg-violet-100 border-violet-400 text-violet-950";
}

const EVENTO_COLOR_MAP: Record<
	EventoColor,
